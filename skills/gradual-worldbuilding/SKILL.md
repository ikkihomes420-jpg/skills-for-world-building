...
It decides **when** a world is allowed to move forward and **what evidence it must show** to do so. The sibling skills decide **how** the resulting material is stored, simulated, researched, and tested:

| Sibling skill | It owns | This skill supplies | This skill consumes |
|---|---|---|---|
| `living-canon-os` | Durable canon storage, statuses, transactions, snapshots, retrieval | Increments formatted as `state-transition`, `change-set`, `consequence`, `claim`, and `decision` records | Approved/provisional status, active snapshot, open questions |
| `storycraft-os` | World systems, rules, factions, simulation, audits, releases | The sequencing of system development and the stage-gate a system must pass | Existing rules, systems, actor states, active consequences |
| `worldbuilding-research` | Evidence-first source records and claims | The specific plausibility questions that require research before a gate opens | `source` records and `claim` records with calibrated confidence |
| `conlang-design` | Languages as testable, versioned systems | The world-level constraints a language must satisfy, and the naming/literacy stage it has reached | Language canon, typological constraints, glossed examples |

Read `references/integration-map.md` for the exact field-level mapping between this skill's loop and each sibling's records, tools, and statuses. Read `references/stage-gates.md` before assessing whether a layer is complete. Read `references/anti-rush-and-repair.md` when a proposal feels too large, too fast, or too convenient.
...
> **A stage is a set of capacities, not a title, a date, or a population number.** The label "city", "kingdom", "religion", or "army" is earned by what a community can reliably do, not by naming it.
...
Also activate it as a **cooling layer** when the user proposes a large jump — a sudden empire, a world religion, a standing army, a capital city, an industrial revolution, or a vanished civilization — and the prerequisites are not yet visible. State plainly which enabling conditions are missing and offer the smallest credible next increment instead of rejecting the idea outright.
...
11. **Show maintenance, not just creation.** A thing that cannot be repaired, resupplied, staffed, or taught is a one-time feat, not a capability.
12. **Let costs land on named people.** Every advancement consumes labor, land, time, health, status, or legitimacy; say who supplies it and who is displaced or indebted.
13. **Keep dialects of scale honest.** Use the world's own words for its own institutions where possible, and do not import later-state vocabulary ("nation", "bureaucracy", "industry", "police") for an earlier stage without justification.
14. **Prefer the mundane bottleneck.** Storage, water, fodder, disease, seasonality, transport time, and inheritance rules usually matter more than battles and edicts.
...
If the project is unclear, reconstruct the smallest reliable state from the available notes before adding new lore. If a `living-canon-os` bundle exists, read its active snapshot and newest session delta first, and use the sibling skill's own status vocabulary (`approved`, `provisional`, `disputed`, `deprecated`, `superseded`) rather than inventing new labels.
...
Select the pressure that is **most forced by the current state** — the bottleneck the community cannot keep deferring. Name the second and third candidate pressures but do not develop them yet; they become the next increments.
...
Do not award an institution, technology, route, title, or military capability until its prerequisites are shown or intentionally documented as an exception. When a prerequisite is genuinely unknown, mark it as an open question instead of silently importing it — and route it to `worldbuilding-research` if a real-world answer would settle it.
...
Do not force all lenses into equal importance. Use the lenses needed to expose hidden assumptions. Where a lens reveals a gap that the sibling skills already model — information horizons (`storycraft-os` knowledge items), rule scope and exceptions (`storycraft-os` world rules), actor asymmetry (`living-canon-os` actor-state) — express the result in that record's vocabulary instead of inventing a parallel one.
...
Include at least one actor who benefits, one who bears a cost, and one who interprets the change differently. Avoid making every group equally powerful, equally tolerant, or equally informed. This maps directly onto `living-canon-os` `actor-state` (objective, capabilities, vulnerabilities, resources, constraints, public position, private agenda) and `knowledge` (proposition, holders, channel, reliability).
...
Use calibrated language: “most plausible,” “possible but costly,” “requires an exception,” or “not credible under current capacity.” Make a recommendation when the evidence favors one path, while preserving the author’s authority over canon. Match the strength of the claim to its support, as `worldbuilding-research` requires: “attested in the sources I found” is not “universal”.
...
Do not convert every consequence into a new event immediately. Some should remain latent until a later pressure activates them. Record those as `living-canon-os` `consequence` records using its states (`latent`, `developing`, `visible`, `transformed`, `resolved`) and scales (`personal`, `relational`, `institutional`, `structural`, `civilizational`).
...
Use `references/stage-gates.md` for the **observable thresholds** behind each of these checks — the measurable capacity indicators, the review questions, and the ladder rungs — and copy `templates/stage-gate.yaml` to record a gate assessment. A gate is passed on demonstrated capacity, not on elapsed time, population, or the author's impatience.
...
When a `living-canon-os` bundle is in use, format the handoff as the records that skill already defines — copy `templates/development-increment.yaml` in this skill, which packages a `state-transition`, a `change-set` when canon moves, created/resolved `consequence`s, `claim`s for anything still provisional, and `decision`/`open-question` entries. Hand it to the canon transaction workflow (`capture_transaction.py` → `review_transaction.py` → `apply_transaction.py`). See `references/integration-map.md` for the field-level mapping.

## Development ladders

Use ladders as diagnostic tools, not mandatory formulas. Each rung below is a **capacity**, not a label; `references/stage-gates.md` gives the observable indicators and the failure signs for every rung.
...
Model both solidarity and pressure. Religious communities may cooperate internally, compete for converts, provide welfare, and create access to transregional networks at the same time. Route doctrinal truth questions to `living-canon-os` knowledge records so belief and world-truth stay separate.
...
`spoken-only contact → borrowed names and terms → shared trade jargon → oral record specialists → restricted literacy → scribal or chancery practice → standardized written register → formal language codification`

Advance this ladder only when the people who use the language can support the rung: a written register needs scribes, a material to write on, a reason to keep records, and readers. Do not let a fully codified grammar appear in a settlement with no literate class. Where language development becomes the actual task, hand the design to `conlang-design` and record its constraints as world rules.

### Knowledge and information

`direct observation → word of mouth → standing witness or memory-keeper → durable record → archive or register → controlled circulation → public account → systematic inquiry`

Information must travel through a plausible channel. Delay, distortion, secrecy, and competing accounts are features, not noise; store them as `knowledge` records with a `reliability` value and holder list.
...
When evidence is thin, narrow the claim instead of inventing false precision. Any claim that a real-world fact supports a world decision should pass through `worldbuilding-research` as a `source` record plus a `claim` with its `reliability`, `use_status`, and calibrated `confidence` — never as an unqualified assertion.
...
Use the answers to refine the proposal rather than automatically rejecting it. `references/anti-rush-and-repair.md` lists the recurring failure modes (grand-title leap, founder fallacy, frictionless borrowing, singular feat, protagonist gravity, cost-free institution) together with their detection signals and repair patterns.
...
### Handoff
List established, changed, open, corrected, next pressure, and canon status — in sibling-skill record form when a canon bundle is in use.

For a deep turn, expand the same structure with actor tables, a timeline, evidence notes, dependency checks, a stage-gate assessment, or a change set supplied by the project’s canon system.
...
- the increment is recorded in the sibling skills' own record vocabulary, not a parallel schema;
- deferred consequences are stored as consequences, not lost as prose;
...
- `references/integration-map.md` — field-level mapping of this skill's loop, ladders, and handoff onto `storycraft-os`, `living-canon-os`, `worldbuilding-research`, and `conlang-design` records, tools, and statuses.
- `references/stage-gates.md` — measurable capacity thresholds, review questions, ladder rungs, and a worked gate assessment.
- `references/anti-rush-and-repair.md` — recurring failure modes, detection signals, and repair patterns.
- `templates/development-increment.yaml` — increment handoff in `living-canon-os` record form.
- `templates/stage-gate.yaml` — stage-gate assessment record.
